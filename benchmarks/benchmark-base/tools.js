        ...
        ${points
          .map(point => {
            ...
            const heapTotal = Math.round(
              data[point].memory.heapTotal / 1024 / 1024
            )
            const heapUsed = Math.round(
              data[point].memory.heapUsed / 1024 / 1024
            )
...
export function defaultCsvReports(data) {
  return {
    ...
    memoryRss: prepareCsv(
      data,
      info => `${Math.round(info.memory.rss / 1024 / 1024)}`
    ),
