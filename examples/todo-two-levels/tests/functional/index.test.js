...
test('should add first list', async (t) => {
    ...
    await t.expect(await Selector('ol').find('li').nth(0).find('a').innerText).eql('List 1');
    
    await t.expect(await Selector('ol').find('li').count).eql(1);
    
    firstListId = (await Selector('ol').find('li').nth(0).find('a').attributes).href.slice(1)
});
...
test('should add second list', async (t) => {
    ...
    await t.expect(await Selector('ol').find('li').nth(1).find('a').innerText).eql('List 2');
    
    await t.expect(await Selector('ol').find('li').count).eql(2);
    
    secondListId = (await Selector('ol').find('li').nth(1).find('a').attributes).href.slice(1)
...
test('should add first todo', async (t) => {
    ...
    await t.expect(await Selector('ol').find('li').nth(0).find('label').innerText).eql('Test 1');
});
...
test('should add second todo', async (t) => {
    ...
    await t.expect(await Selector('ol').find('li').nth(1).find('label').innerText).eql('Test 2');
});

test('should toggle first todo', async (t) => {
    await t.expect(await Selector('input[type=checkbox]').nth(0).checked).eql(false);

    await t.click(await Selector('input[type=checkbox]').nth(0));

    await t.expect(await Selector('input[type=checkbox]').nth(0).checked).eql(true);
});

test('should remove first todo', async (t) => {
    await t.expect(await Selector('ol').find('li').count).eql(2);

    await t.click(await Selector('span').withText('[x]').nth(0));
