...
export default {
  ...
  [USER_CREATED]: async (store, {  payload: { userId } }) => {
    await store.insert('UserActions', { userId, upvoteCount: 0, unvoteCount: 0, storyCount: 0, commentCount: 0 })
  },
  ...
  COMMENT_CREATED: async (store, { payload: { authorId } }) => {
    await store.update('UserActions', { userId: authorId  }, { $inc: { commentCount: 1 } })
  }
}
