...
const resolveScriptsPath = path.join(
  path.dirname(require.resolve('resolve-scripts')),
  '..'
)
const resolveReduxPath = path.join(
  path.dirname(require.resolve('resolve-redux')),
  '..'
)
...
execSync(`yarn --registry="http://npm.resolve.sh:10080"`, {
  cwd: shoppingListPath,
  stdio: 'inherit'
})
