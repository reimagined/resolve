...
describe('resolve-readmodel-memory store-api', () => {
  ...
  it('should provide defineTable method', async () => {
    ...
    expect(newStorage.ensureIndex.firstCall.args[0].fieldName).toEqual('first')
    expect(newStorage.ensureIndex.secondCall.args[0].fieldName).toEqual(
      'second'
    )
    expect(newStorage.ensureIndex.thirdCall.args[0].fieldName).toEqual('third')
    expect(storage['test']).toEqual(newStorage)
  })
  ...
  it('should provide find method - all arguments passed', async () => {
    ...
    expect(cursor.sort.firstCall.args[0]).toEqual({ sort: -1 })
    expect(cursor.skip.firstCall.args[0]).toEqual(10)
    expect(cursor.limit.firstCall.args[0]).toEqual(20)
  ...
  it('should provide find method - no projection passed', async () => {
    ...
    expect(cursor.projection.firstCall.args[0]).toEqual({ _id: 0 })
    expect(cursor.sort.firstCall.args[0]).toEqual({ sort: -1 })
    expect(cursor.skip.firstCall.args[0]).toEqual(10)
    expect(cursor.limit.firstCall.args[0]).toEqual(20)
  ...
  it('should provide find method - no sort passed', async () => {
    ...
    expect(cursor.sort.callCount).toEqual(0)
    expect(cursor.skip.firstCall.args[0]).toEqual(10)
    expect(cursor.limit.firstCall.args[0]).toEqual(20)
  ...
  it('should provide find method - no skip passed', async () => {
    ...
    expect(cursor.sort.firstCall.args[0]).toEqual({ sort: -1 })
    expect(cursor.skip.callCount).toEqual(0)
    expect(cursor.limit.firstCall.args[0]).toEqual(20)
  ...
  it('should provide find method - no limit passed', async () => {
    ...
    expect(cursor.sort.firstCall.args[0]).toEqual({ sort: -1 })
    expect(cursor.skip.firstCall.args[0]).toEqual(10)
    expect(cursor.limit.callCount).toEqual(0)
  ...
  it('should provide insert method', async () => {
    ...
    expect(storage.test.insert.firstCall.args[0]).toEqual({
      id: 1,
      value: 2
    })
  })
  ...
  it('should provide update method', async () => {
    ...
    expect(storage.test.update.firstCall.args[1]).toEqual({
      id: 1,
      value: 10
    })
  })
