...
export const assertLeadingSlash = (value: string, name?: string): void => {
  if (!value?.startsWith('/')) {
...
export const getStaticBasedPath = (
  rootPath: string,
  staticPath: string,
  filename: string
) => {
...
function escapeUnicodeSymbol(match: string) {
  const code = match.codePointAt(0) ?? 0
  return `\\u${code.toString(16).padStart(4, '0')}`
}

export const jsonUtfStringify = function (inputObject: object) {
