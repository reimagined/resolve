  ...
  new Promise((resolve, reject) => {
    const params = [
      ...
      (err, data) => {
        ...
        return resolve(
          Array.isArray(data) ? data.map(safeParse) : safeParse(data)
        )
      }
    ]
...
export const hget = async (client, key, field) =>
  await invokeCommand(client, 'HGET', key, field)

export const hkeys = async (client, key) =>
  await invokeCommand(client, 'HKEYS', key)
