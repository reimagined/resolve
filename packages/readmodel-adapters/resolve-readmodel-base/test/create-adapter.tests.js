...
describe('resolve-readmodel-base create-adapter', () => {
  it('should work properly - on apropriate storeApi and metaApi', async () => {
    const [buildProjection, init, reset] = [sinon.stub(), sinon.stub(), sinon.stub()]
    ...
    expect(implementation.firstCall.args[0].metaName).to.be.equal(options.metaName)
    ...
    expect(buildProjection.firstCall.args[0].storeApi).to.be.equal(checkedStoreApi)
  ...
  it('should throw error - on invalid storeApi and metaApi', async () => {
    ...
    implementation.onCall(0).callsFake(() => ({ metaApi: null, storeApi: null }))
    implementation.onCall(1).callsFake(() => null)

    try {
      createAdapter(sinon.stub(), sinon.stub(), sinon.stub(), sinon.stub(), implementation, {})
      return Promise.reject('Adapter creation should fail on invalid storeApi and metaApi')
    } catch (err) {
