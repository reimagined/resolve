...
describe('Read model MongoDB adapter', () => {
  ...
  beforeEach(async () => {
    ...
    const createCollection = async collectionName => {
      ...
      const voidExecutor = (resolve, reject, err) =>
      ...
      return {
        find: searchPattern => ({
          toArray: toArrayImpl.bind(null, searchPattern)
        }),
        findOne: (...args) =>
          new Promise(unaryWrapper.bind(null, 'findOne', args)),
        count: (...args) => new Promise(unaryWrapper.bind(null, 'count', args)),
        insert: (...args) =>
          new Promise(voidWrapper.bind(null, 'insert', args)),
        update: (...args) =>
          new Promise(voidWrapper.bind(null, 'update', args)),
        remove: (...args) =>
          new Promise(voidWrapper.bind(null, 'remove', args)),
        ...
        dropIndex: (...args) =>
          new Promise(voidWrapper.bind(null, 'removeIndex', args)),
        drop: async () => {
  ...
  describe('Disposing adapter with reset function', () => {
    ...
    beforeEach(async () => {
      await testConnection.collection(
        `${COLLECTIONS_PREFIX}${DEFAULT_DICTIONARY_NAME}`
      )
    ...
    it('should close connection and dispose target dictionarys', async () => {
      ...
      expect(
        storedCollections[`${COLLECTIONS_PREFIX}${META_COLLECTION_NAME}`]
      ).to.be.equal(undefined)
