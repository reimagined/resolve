...
const checkFormalError = (error: PassthougthErrorLike, value: string) : boolean => (
...
const PassthroughError: PassthroughErrorFactory = Object.assign(
  (function (this: PassthroughErrorInstance, lastTransactionId: string, isRetryable: boolean): void {
    Error.call(this)
  ...
  } as Function) as ExtractNewable<PassthroughErrorFactory>,
  {
    ...
    isRetryablePassthroughError(error: PassthougthErrorLike) : boolean {
      return (
        ...
        checkFuzzyError(error, /could not serialize access due to concurrent update/i) ||
    ...
    isRegularFatalPassthroughError(error: PassthougthErrorLike) : boolean {
      return (
        error != null &&
        (checkFuzzyError(error, /could not obtain lock/i) 
        )
      )
    },
    ...
    isPassthroughError(error: PassthougthErrorLike, includeRuntimeErrors = false): boolean {
      return !!includeRuntimeErrors && PassthroughError.isRuntimeFatalPassthroughError(error) &&
