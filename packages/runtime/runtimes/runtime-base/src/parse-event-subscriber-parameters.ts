const parseEventSubscriberParameters = <T extends {
    eventSubscriber?: string | null | undefined,
    modelName?: string | null | undefined,
}>(params: T) : [string, Omit<T, 'eventSubscriber' | 'modelName'>] => {
    if (Object(params) !== params) {
      throw new TypeError(
        `Arguments of resolve-query method should be object, but got: ${JSON.stringify(params)}`
      )
    }
