import { createAuthOptions, createRequest, createResponse, resolveAuth } from 'resolve-auth'
...
const authStrategies = authStrategiesConfigs.map(({ strategyConstructor, options }) =>
  resolveAuth(strategyConstructor, options)
)

const assignAuthRoutes = app => {
  authStrategies.forEach(({ route, callback }) => {
    app[route.method.toLowerCase()](getRootBasedUrl(route.path), (req, res, next) => {
