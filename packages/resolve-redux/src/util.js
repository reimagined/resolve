...
export function getRootableUrl(path) {
  let rootDir =
    typeof process !== 'undefined' && typeof process.env !== 'undefined' && process.env['ROOT_PATH']
...
export function makeLateResolvingPromise(...args) {
  if (args.length > 1) {
    throw new Error('Make late-resolved promise accepts only zero on one argument')
  }
