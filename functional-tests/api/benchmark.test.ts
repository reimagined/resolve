...
const isHighloadError = (error: Error) =>
  error != null &&
  /(?:HTTP ERROR 503)|(?:ETIMEDOUT)|(?:ECONNRESET)|(?:EMFILE)|(?:sorry, too many clients already)|(?:remaining connection slots are reserved for non-replication superuser and rds_superuser connections)|(?:Connection terminated unexpectedly)/.test(
    error.message
  )
...
const getEventCount = async (benchType: 'lite' | 'heavy') => {
  return +((await performApiPost(
    `/query/benchmark-${benchType}/countBenchEvents`,
    {
      data: -1,
    }
  )) as any).data
...
test('benchmark', async () => {
  ...
  while (generatedBenchEventsCount < BENCH_EVENTS_COUNT) {
    const currentGeneratedEvents = (
      await Promise.all(Array.from({ length: 8 }).map(generateEvents))
